...
def getCarData(filename="./input/carname.txt"):
    ...
    for line in lines:
        ...
        car_data = pd.read_csv(
            filepath_or_buffer=input_car_file, error_bad_lines=False)
...
def read_data(filename="./input/carname.txt"):
    ...
    for line in lines:
        ...
        # print(car_name)
        input_car_file = "./input/MTA_Tracking/" + car_name
        t_min_x, t_max_x, t_min_y, t_max_y, name, positions = read_csv_data(
            input_car_file)
...
def read_csv_data(filename):
    ...
    longDF = car_data['VehicleLocation.Longitude'] / abs(long).max()
    latDF = car_data['VehicleLocation.Latitude'] / abs(lat).max()
    newDF = pd.concat([time_recored, longDF, latDF], axis=1)
    newDF.to_csv("input/norm/" + car_name + ".csv", index=False)

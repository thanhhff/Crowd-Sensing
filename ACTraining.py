...
acAgent = ActorCritic(param.num_car, dqn_conf.INPUTNUM,
                      param.n_size, dqn_conf.ACTIONNUM)
...
header = ["Episode", "Reward", "Running_reward",
          "Cover_area", "Sharing_factor", "Sent_pkg"]
            ...
            with tf.GradientTape() as tape:
                ...
                for i in range(param.num_car):
                    if ((step * param.step_length) % acAgent.action_steps[i] == 0):
                        ...
                        map_state = acAgent.knowledges[i].build_matrix_state(
                            acAgent.action_steps[i])
                        map_state = tf.convert_to_tensor(
                            map_state, dtype=tf.float32)
                ...
                if (len(acted_agents) != 0):
                    reward, cover_area, sharing_factor, sent_factor = net.step(
                        actions, step, episode_i, test=True)
